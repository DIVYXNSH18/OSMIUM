...
pub async fn handle(ctx: &Context, channel: &GuildChannel, data: Arc<BotData>) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    if config.antinuke.enabled {
        if let Ok(audit_logs) = guild_id.audit_logs(&ctx.http, Some(serenity::model::guild::audit_log::Action::ChannelDelete), None, None, Some(1)).await {
    ...
    if let Some(log_channel) = config.log_channel_id {
        ...
        if let Err(e) = logger.log_channel_delete(channel).await {
            error!("Failed to log channel delete: {}", e);
        }
    }
